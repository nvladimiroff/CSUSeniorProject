...
class Spice {
    ...
    public function findSimilar($str) {
        ...
        $sql = "select * from spices where name like '%".$db->real_escape_string($str)."%' and status = 1";
        $result = $db->query($sql);
    ...
    static function addNewIngredientIfNeeded($ingredients) {
        ...
        foreach($arr as $ingredient) {
            ...
            $sql = "Select * from spices where name = '".$ingredient."' and status = 1";
            if ($result = $db->query($sql)) {
                $row = $result->fetch_assoc();
                if (empty($row)) {
                    $insertSql = "INSERT INTO spices (`name`,`description`,`created`,`modified`) VALUES('".$ingredient."','".$ingredient."',now(), now())";
